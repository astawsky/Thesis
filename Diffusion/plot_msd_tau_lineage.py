...
def main():
    # physical_units = pd.read_csv('Data/physical_units.csv')
    ...
    for param in phenotypic_variables:
        ...
        for count, kind in enumerate(['Population', 'Trace-Centered', 'Shuffled TC']): # 'Shuffled', 'Trace',
            ...
            # slope, intercept, _, _, std_err = linregress(np.log(to_plot['generation'].iloc[::-1].values), np.log(to_plot['var'].values))
            #
            # plt.scatter(to_plot['generation'].iloc[::-1].values, to_plot['var'].values, label=kind+r'$: {:.2} \pm {:.2}$'.format(slope, std_err), color=cmap[count], marker=markers[count], alpha=.7)
            # plt.plot(to_plot['generation'].iloc[::-1].values, np.exp([intercept + slope * np.log(gen) for gen in to_plot['generation'].iloc[::-1]]), ls='--', color=cmap[count])
