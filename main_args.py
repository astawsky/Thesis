...
# Run all the dataframe creation
def main(data_origin='SM'):
    # Based on the kind
    args = get_args(data_origin)
...
# Get args for a specific type of data
def get_args(data_origin):
    # Create the folders where we will save them
    create_folder(os.path.dirname(os.path.abspath(__file__)) + '/Data/'+data_origin)
    create_folder(os.path.dirname(os.path.abspath(__file__)) + '/Figures/'+data_origin)
    
    parser = argparse.ArgumentParser(description='Data Pipeline for the Mother Machine and Sister Machine Traces')
    
    # SM, MG1655_inLB_LongTraces, Maryam_LongTraces, Lambda_LB, LAC_M9 are the options we use
    parser.add_argument('-data_origin', '--data_origin', metavar='', type=str, help='What is the label for this data for the Data and Figures folders?', required=False, default=data_origin)
    
    # For the two types of dataframes: SM and MM
    if data_origin == 'SM':
    ...
    parser.add_argument('-raw_data', '--raw_data', metavar='', type=str, help='Raw Data location.', required=False, default=os.path.dirname(os.path.abspath(__file__)) + '/RawData/' + data_origin)
    parser.add_argument('-SL', '--sp_infiles', metavar='', type=str,
                        help='Location of Sister Pair Raw Data', required=False, default=os.path.dirname(os.path.abspath(__file__)) + '/RawData/SM/SL')
    parser.add_argument('-NL', '--nc_infiles', metavar='', type=str,
                        help='Location of Neighboring Cell Pair Raw Data', required=False, default=os.path.dirname(os.path.abspath(__file__)) + '/RawData/SM/NL')
