...
def pyramid_of_pairwise_covariances():
    ...
    plt.savefig('{}/main variables without blanks.png'.format(args.figs_location), dpi=300)
    # plt.show()
    plt.close()


def pyramid_no_numbers_in_cells():
    ...
    sns.heatmap(pu, annot=False, center=0, vmax=vmax,
                vmin=vmin, cbar=False, ax=axes[0], mask=mask, square=True, fmt='.2f')
    axes[0].set_title('A', x=-.2, fontsize='xx-large')
    
    sns.heatmap(ta, annot=False, center=0, vmax=vmax, vmin=vmin,
                cbar=False, ax=axes[1], mask=mask, square=True, fmt='.2f')
    axes[1].set_title('B', x=-.2, fontsize='xx-large')
    
    cbar_ax = fig.add_axes([.91, .1, .03, .8])
    
    sns.heatmap(tc, annot=False, center=0, vmax=vmax,
                vmin=vmin, cbar=True, ax=axes[2], mask=mask, cbar_kws={"orientation": "vertical"}, square=True, cbar_ax=cbar_ax, fmt='.2f')
    axes[2].set_title('C', x=-.2, fontsize='xx-large')
    
    fig.tight_layout(rect=[0, 0, .9, 1])
    
    plt.savefig('{}/main variables without numbers.png'.format(args.figs_location), dpi=300)
    # plt.show()
    plt.close()
...
physical_units = pd.read_csv(args.pu)[variables]

# read the csv file where we keep the data
trace_centered, trace_means_df = pd.read_csv(args.tc)[variables], pd.read_csv(args.ta)[variables]

pyramid_of_pairwise_covariances()

pyramid_no_numbers_in_cells()
