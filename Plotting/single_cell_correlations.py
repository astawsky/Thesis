...
def put_all_graphs_into_a_big_grid(df, label, variables=phenotypic_variables, remove_outliers=False):
    ...
    plt.savefig('{}/{}.png'.format(args.scc, label), dpi=300)
    # plt.show()
    plt.close()
...
def heatmap_analogs(df, label, variables=phenotypic_variables):
    ...
    # plt.title(label)
    # plt.tight_layout()
    plt.savefig('{}/{}.png'.format(args.scch, label), dpi=300)
    # plt.show()
    plt.close()
...
create_folder(args.scc)
create_folder(args.scch)

# import the labeled measured bacteria in trace-centered units
physical_units = pd.read_csv(args.pu).sort_values(['dataset', 'trap_ID', 'trace'], ascending=[1, 1, 1])
trace_centered = pd.read_csv(args.tc).sort_values(['dataset', 'trap_ID', 'trace'], ascending=[1, 1, 1])
time_averages = pd.read_csv(args.ta).sort_values(['dataset', 'trap_ID', 'trace'], ascending=[1, 1, 1])
