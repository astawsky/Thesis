...
from CustomFuncsAndVars.global_variables import phenotypic_variables, boot_pearson
...
def intergenerational_creations(info_columns, info, how_many_separations, label, n_boots):
    ...
    for separation in range(1, how_many_separations + 1):
        print('inter-generational distance of {}'.format(separation))
...
def main(args):
    ...
    print('pu')
    total_inter_df = intergenerational_creations(physical_units.columns, physical_units, how_many_separations=15, label='physical units', n_boots=args.bs).reset_index(drop=True)
    print('tc')
    total_inter_df = total_inter_df.append(intergenerational_creations(trace_centered.columns, trace_centered, how_many_separations=15, label='trace centered', n_boots=args.bs),
                                           ignore_index=True).reset_index(drop=True)
    
    total_inter_df.to_csv('{}/{}'.format(args.save_folder, args.inter), index=False)
