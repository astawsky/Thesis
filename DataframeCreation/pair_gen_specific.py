...
# def generation_specific(bootstraps=False):
...
#     df = pd.DataFrame(
#         columns=['variable_a', 'variable_b', 'AB_parameter_pair', 'covariance', 'correlation', 'kind', 'average over lineages, trace A',
#                  'average over lineages, trace B', 'cov difference between data and model' 'dataset', 'generation'])
#
#     # dataframe that we will save
#     bs = pd.DataFrame(
#         columns=['variable_a', 'variable_b', 'AB_parameter_pair', 'covariance', 'correlation', 'kind', 'average over lineages, trace A',
...
#
#                         tc_a = trace_centered[(trace_centered['dataset'] == dataset) & (trace_centered['generation'] == generation)].sort_values(['trace', 'trap_ID'], ascending=[True, True])[
#                             variable_a].reset_index(drop=True)
#                         tc_b = trace_centered[(trace_centered['dataset'] == dataset) & (trace_centered['generation'] == generation)].sort_values(['trace', 'trap_ID'], ascending=[False, True])[
#                             variable_b].reset_index(drop=True)
#
...
#
#                         for kind, a, b, cov, corr in [
#                             ('physical_units', np.mean(pu_a), np.mean(pu_b), pu_cov, pu_corr),
#                             ('trace_centered', np.mean(tc_a), np.mean(tc_b), tc_cov, tc_corr),
#                             ('time_averages', np.mean(ta_a), np.mean(ta_b), ta_cov, ta_corr),
#                             ('model', np.mean(ta_a + tc_a), np.mean(ta_b + tc_b), actual_cov, actual_corr)
#                         ]:
#
#                             # the labels
...
#                                 'correlation': corr,
#                                 'kind': kind,
#                                 'average over lineages, trace A': a,
#                                 'average over lineages, trace B': b,
...
#                                     indices]
#                                 tc_b = \
#                                     trace_centered[(trace_centered['dataset'] == dataset) & (trace_centered['generation'] == generation)].sort_values(['trace', 'trap_ID'], ascending=[False, True])[
#                                         variable_b].reset_index(drop=True).iloc[
#                                         indices]
#
#                                 ta_a = trace_means_df[(trace_means_df['dataset'] == dataset) & (trace_means_df['generation'] == generation)].sort_values(['trace', 'trap_ID'], ascending=[True, True])[
        ...
        for variable_a in phenotypic_variables:
            ...
            for variable_b in phenotypic_variables:
                ...
                if variable_b == variable_a:
                    ...
                    a_std = np.std(physical_units[(physical_units['dataset'] == dataset)][variable_a])
                    b_std = np.std(physical_units[(physical_units['dataset'] == dataset)][variable_b])
...
pair_correlation, bootstrapped = generation_specific(bootstraps=args.bootstraps)
...
if not args.bootstraps:
    ...
    bootstrapped.to_csv('{}/over_lineages_{}_bootstraps.csv'.format(args.save_folder, args.bootstraps), index=False)
