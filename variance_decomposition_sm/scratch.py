...
def vd_with_trap_and_lineage(save_fig, variables):
    ...
    for type_of_lineages, lin_type in zip([['NL'], ['SL'], ['SL', 'NL']], ['NL', 'SL', 'SL and NL']):
        ...
        conds = (output_df['kind'] == 'Artificial') & (~output_df['variable'].isin(['', 'before', 'after', 'between']))
...
if __name__ == '__main__':
    ...
    vd_with_trap_and_lineage(os.path.dirname(os.path.abspath(__file__)),
    ...
    vd_with_lineage_and_experiments(['Lambda_LB', 'Maryam_LongTraces'], os.path.dirname(os.path.abspath(__file__)), lin_type='wang_MG1655')  #  + sm_datasets[:-1]
    exit()
    #
