...
def recreate_loglog_plots(loglog_plot_recreation, ax):
    
    print(loglog_plot_recreation.variable.unique())
    ...
    print(np.arange(5, 83, dtype='int'), np.log(.47/(5**.5)) * (np.arange(5, 83) ** .5))
...
recreation = pd.read_csv(r'/Users/alestawsky/PycharmProjects/Thesis/DFA/LogLog Recreation/Pooled_SM/loglog_scaling_recreation.csv')
...
recreate_loglog_plots(recreation[recreation['kind'] == 'dfa (short)'], axes[1])
plot_dfa_slopes(axes[2])
plt.savefig('Figures/dfa figure updated.png', dpi=500)
# plt.show()
plt.close()
